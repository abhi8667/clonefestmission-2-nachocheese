export function validateFlagCreation(flagType, setterRole, setterId, requesteeId) {
    ...
    const canRequest = setterRole === flagType.request_role ||
        setterRole === 'admin' ||
...
export function validateFlagResolution(flag, flagType, actorId, actorRole, newStatus) {
    if (flag.status !== '?') {
        return {
            valid: false,
    ...
    const hasGrantRole = actorRole === flagType.grant_role ||
