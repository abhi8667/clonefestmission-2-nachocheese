...
import {
  ...
  createTransitionActivity
} from '../src/workflow.js';
import {
  validateFlagCreation,
  validateFlagResolution
} from '../src/flags.js';
import {
  computeTimeInState,
  deriveFlowMetrics,
  detectStalledState,
  detectSleeperBranches,
...
import { Flag, FlagType, Bug, Activity, GitLink } from '@triarc/shared-types';
...
describe('Engine Relationship Validator', () => {
  it('Rejects self-blocking and circular blocking', () => {
    ...
    const existing = [
      { id: 1, from_bug_id: 10, to_bug_id: 20, type: 'BLOCKS' as const, created_at: '' }
    ];
