...
export function runSeed() {
  ...
  `);
    ...
    insertBug.run(398, 'Login fails on mobile Safari with cookies disabled', 'Authentication session handshake returns 401 loop when third-party cookie restrictions are active on iOS Safari 17.5+.', 'Confirmed', 'major', 'high', 'auth', 'u_chen', 'u_sam', new Date(now - 2 * DAY_MS).toISOString(), new Date(now - 6 * HOUR_MS).toISOString());
    ...
    insertBug.run(250, 'Refactor SSE connection recovery and exponential backoff', 'Improve reconnection jitter and handle network sleep wake-up events on background tab restoration.', 'In Progress', 'normal', 'normal', 'api', 'u_priya', 'u_alex', new Date(now - 8 * DAY_MS).toISOString(), new Date(now - 5 * DAY_MS).toISOString());
    ...
    insertBug.run(999, 'Potential JWT token confusion with asymmetric public keys', 'Algorithm validation must reject none and HMAC when RS256 key is expected in verification headers.', 'In Progress', 'blocker', 'highest', 'auth', 'u_sarah', 'u_marcus', new Date(now - 3 * DAY_MS).toISOString(), new Date(now - 1 * DAY_MS).toISOString());
    ...
    for (let i = 1; i <= 140; i++) {
        ...
        if (i % 7 === 0)
            ...
            status = 'Duplicate';
        const result = insertBug.run(null, title, `Detailed investigation report for issue #${i}. This occurs intermittently during high volume batch processing and requires verification of state invariants across asynchronous worker boundaries.`, status, severity, priority, component, reporter, assignee, createdIso, new Date(createdDate.getTime() + 1.5 * DAY_MS).toISOString());
    ...
    const totalBugs = db.prepare('SELECT COUNT(*) as count FROM bugs').get();
    const totalActivities = db.prepare('SELECT COUNT(*) as count FROM activity').get();
    const totalFlags = db.prepare('SELECT COUNT(*) as count FROM flags').get();
