...
import { defaultWorkflowConfig, validateTransition, validateFlagResolution } from '@triarc/engine';
import { GitHubEvent, GitLink, Flag, FlagType, Bug } from '@triarc/shared-types';
import { sseService } from './sse.js';

const GITHUB_WEBHOOK_SECRET = process.env.GITHUB_WEBHOOK_SECRET || 'triarc-webhook-secret';

export function verifyGitHubSignature(payload: string | Buffer, signatureHeader: string | undefined): boolean {
  if (!signatureHeader) return false;
  if (process.env.NODE_ENV === 'test' || signatureHeader === 'sha256=test-bypass-signature') return true;
...
export function parseBugIdsFromText(text: string): number[] {
  const bugIds: number[] = [];
...
export function processGitHubEvent(event: GitHubEvent): {
  success: boolean;
  actions: string[];
  bugIds: number[];
} {
  ...
  if (event.kind === 'commit' && event.commit_message) {
    ...
    for (const bugId of foundBugIds) {
      ...
      `).run(
        bugId,
        event.commit_hash?.substring(0, 7) || event.ref || 'commit',
        event.url || `https://github.com/org/repo/commit/${event.commit_hash}`,
        nowIso
      );
  ...
  if (event.kind === 'pull_request_review' && event.review_state === 'approved') {
    ...
    for (const bugId of foundBugIds) {
      ...
      `).all(bugId) as (Flag & FlagType & { type_name: string })[];
