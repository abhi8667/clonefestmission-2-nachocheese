...
analyticsRouter.get('/analytics/flow', (req: AuthenticatedRequest, res: Response) => {
  ...
  const totalResolved = db.prepare("SELECT COUNT(*) as count FROM bugs WHERE status IN ('Resolved', 'Verified', 'Closed')").get() as { count: number };
